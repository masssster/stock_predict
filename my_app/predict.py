...
def predict(saham,waktu,model_type):
    ...
    with st.container():
        ...
        if(model_type == "Neural"):
            ...
            st.write("MAPE : " + mean_absolute_percentage_error(forecast['yhat1'][forecast["ds"] <= "2022-01-01"],df['y']))
            ...
            forecast = model.predict(future)  
            st.write("MAPE : " + mean_absolute_percentage_error(forecast['yhat'][forecast["ds"] <= "2022-01-01"],df['y'])) 
            fig = plt.figure(figsize=(16, 9),dpi=100)
            plt.plot(df['ds'], df['y'], 'y' ,label = "Actual")         
            plt.plot(df['ds'], forecast['yhat'][forecast["ds"] <= "2022-01-01"], 'k', label = "Predicted")    
            plt.plot(forecast['ds'], forecast['yhat'][forecast["ds"] >= "2022-01-01"], 'k', label = "Predicted")     
            plt.legend()
            fig_html = mpld3.fig_to_html(fig)
            components.html(fig_html, height=1000, width= 1600)  
            forecast = forecast[forecast["ds"] >= "2022-01-01"]
